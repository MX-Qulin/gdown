...
from os import path, mkdir, getcwd, chdir, system
...
class Main:
    ...
    def _downloadContent(url: str, token: str, chunk_size: int = 4096) -> None:
        ...
        with get(url, headers=headers, stream=True) as response_handler:
            ...
            with open(filename, 'wb+') as handler:
                ...
                if flag == 1 :
                    os.system(f"mv {filename} {mv_pth}")
    ...
    def _getLinks(url: str, password: str | None = None) -> Generator[str, None, None]:
        ...
        if "contents" in data.keys():
            ...
            die(f"Failed to get link, maybe password wrong or link wrong.")
...
if __name__ == '__main__':
    try:
        ...
        if argc > 1:
            ...
            Main(url="https://gofile.io/d/"+url, password=password)
                
        else:
            die("Usage:"
                + NEW_LINE
                + "python gdown.py (don't input https://gofile.io/d/) [contentid]"
                + NEW_LINE
                + "python gdown.py (don't input https://gofile.io/d/) [contentid] [password]"
            )
